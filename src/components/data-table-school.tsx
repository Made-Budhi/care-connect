...
export function DataTableSchool<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
    const {auth} = useAuth();
    ...
    return (
        <div className={"space-y-3"}>
            <section id="filtering-section" className={"flex gap-2"}>
                {auth.role === 'admin' && (
                    <Link to={"/admin/schools/add"} className={`${buttonVariants({variant: "ccbutton"})}`}>
                        <Plus />
                        <p>NEW</p>
                    </Link>
                )}
